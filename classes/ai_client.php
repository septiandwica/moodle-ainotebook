...
class ai_client {
    ...
    public static function get_response(int $cmid, int $userid, string $user_message, array $selected_file_ids = [], array $config = []): string {
        ...
        $system_prompt .= "7. MINDMAP: Generate a comprehensive English mindmap using Mermaid.js. You MUST wrap the mermaid code inside a code block tagged with 'mermaid' (e.g. ```mermaid graph TD ... ```). [STRICT RULES] 1. For labeled edges, use: 'NodeA -->|Label| NodeB'. 2. NEVER repeat the node ID after the closing bracket (use `Node[Label]`, NOT `Node[Label]Node`). 3. If a label contains parentheses `()`, wrap the label in double quotes like `Node[\"Label (Info)\"]`.\n";
